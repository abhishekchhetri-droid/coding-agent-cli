...
export class LangflowClient {
  ...
  async listFlows(params?: ListFlowsParams): Promise<{ id: string; name: string; description: string | null }[]> {
    try {
      const { search, ...apiParams } = (params ?? {}) as { search?: string; [key: string]: unknown };
      // Default: paginated, not get_all — prevents 5MB dumps
      const safeParams = { page: 1, size: 100, get_all: false, ...apiParams };
      const response = await this.client.get<FlowRead[] | { items: FlowRead[] }>('/flows/', { params: safeParams });
      let flows: FlowRead[] = Array.isArray(response.data)
        ? response.data
        : (response.data as { items: FlowRead[] }).items ?? [];
      if (search) {
      ...
      return flows.map(f => ({ id: f.id, name: f.name, description: f.description ?? null }));
    } catch (error) {
